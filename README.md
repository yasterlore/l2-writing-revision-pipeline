...
- `check-web-logger-rust-validator-position-unit-phase1`: Step-web-logger-049 adds a standalone Makefile target for the Step-web-logger-047 focused Rust validator tests. Step-web-logger-059 corrects the command to `cargo test -p kslog_validate position_unit_phase1` so the target is Phase 1-only again after Phase 2 tests were added. The help text remains `Run Rust validator position_unit Phase 1 policy tests`. The target runs focused validator tests only; it does not run the full validator test suite, workspace tests, Python fixture validator, replay checks, fixture changes, Phase 2 UTF-16 numeric metadata target coverage, extract / micro_episode integration, TypeScript logger changes, SHA-256 helper work, TypeScript/Rust checks, event durability, production readiness, real-data readiness, or model performance evidence.
...
- `check-web-logger-rust-validator-position-unit-phase2-utf16-numeric`: Step-web-logger-059 adds a standalone Makefile target for Rust validator Phase 2 UTF-16 numeric metadata focused tests. The help text is `Run Rust validator position_unit Phase 2 UTF-16 numeric metadata tests`, and the command is `cargo test -p kslog_validate position_unit_phase2`. The target is placed after the corrected Phase 1 validator target and before `check-web-logger-rust-utf16-offset-conversion`. Step-web-logger-059 does not change `scripts/check_release_quality.sh`, Rust code/tests, fixtures, TypeScript/Python code, Cargo/package metadata, workflows, extract / micro_episode integration, SHA-256 helper work, TypeScript/Rust vector checks, event durability, production readiness, real-data readiness, or model performance evidence.
- `scripts/check_release_quality.sh`: Step-web-logger-061 adds release-quality label `release_quality_check: web logger Rust validator position_unit Phase 2 UTF-16 numeric metadata` with command `make check-web-logger-rust-validator-position-unit-phase2-utf16-numeric`. The wrapper calls the Makefile target rather than duplicating the Cargo command directly, and inserts the check after the Phase 1 validator label and before Rust UTF-16 replay integration. Phase 1 label / command remain unchanged. Phase 2 run record workflow, status marker, final safety review, replay correctness proof, extract / micro_episode integration, TypeScript logger changes, SHA-256 helper work, TypeScript/Rust vector checks, event durability, production readiness, real-data readiness, and model performance evidence remain future work.
- `apps/logger-web/src/rawEvent.ts`: Step-web-logger-066 updates the synthetic-only TypeScript logger event shape to emit `logger_schema_version=kslog.raw_event.v2`, `research_schema_target=web_logger_position_unit_schema_v0.1`, and `position_unit=utf16_code_unit`. Document lengths now use JavaScript string `.length` as UTF-16 code unit length, and diff/deletion slicing follows UTF-16 code unit offsets from browser selection APIs. Focused logger-web tests cover metadata emission, UTF-16 lengths for ASCII/Japanese/emoji/combining/newline cases, cursor/selection offset preservation, and no-oracle field absence. This does not change Rust/Python code, fixtures, Makefile, release-quality wrapper, replay behavior, extract / micro_episode integration, SHA-256 helper work, TypeScript/Rust compatibility fixture validation, event durability, production readiness, real-data readiness, or model performance evidence.

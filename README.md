...
- `check-learner-state-frozen-policy-generation-artifact-body-generation-runtime-invocation-fixtures`: Step574 standalone Makefile target for the Step572 runtime invocation fixture validator CLI; expected aggregate is 30 cases / 210 JSON with 6 pass, 1 usage-error, 22 fail-closed, and 1 mismatch case. The target is not yet release-quality integrated and does not invoke artifact body generation runtime, invoke manifest writer, or write files.
...
- `check-learner-state-frozen-policy-generation-artifact-body-generation-runtime-invocation`: Step579 standalone Makefile target for the Step577 planned-only v0.3 runtime invocation smoke; it runs the Step570 primary case and emits the same public-safe metadata-only summary. The target is not connected to the release-quality wrapper and does not invoke actual artifact body generation runtime, invoke manifest writer, or write files.

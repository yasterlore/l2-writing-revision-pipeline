...
to include learner-state estimator input validation.
...
smoke check, and learner-state estimator input validation check through the
...
- included: yes
- result: pass
- `minimal_single_participant`: export status pass, feature row count 3,
  label row count 3, audit status pass
- `past_window_boundary_reset`: export status pass, feature row count 4,
  label row count 4, audit status pass
...
The estimator input validation summary is count-only. Generated
`features.jsonl`, `labels.jsonl`, and `manifest.json` bodies are not included
in this marker.
...
- fixture row contents copied into docs: no
...
included the estimator input validation smoke.
...
- introduce selective prediction or calibration

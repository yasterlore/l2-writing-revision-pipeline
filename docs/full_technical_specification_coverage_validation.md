...
| artifact writer CLI integration runtime module | runtime module, focused tests, Step491 standalone Makefile target, Step493 wrapper inclusion, and Step515 standalone actual invocation metadata-only target | 12, 16, 19, 21 | covered | Step489 plan-only metadata boundary remains default; Step513 adds explicit `actual_invocation_metadata_only` v0.2 summary support; Step515 adds a standalone target for one valid v0.2 fixture case without wrapper integration, file writing, artifact body generation integration, or manifest writer integration. |

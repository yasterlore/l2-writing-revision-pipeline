...
- [Selective prediction and calibration release-quality integration design](selective_prediction_calibration_release_quality_integration_design.md): docs-only wrapper placement, command choice, log safety, runtime impact, failure meaning, and remote-run recording policy for the calibration validator target.
...
- [Selective prediction and calibration validator CLI design](selective_prediction_calibration_validator_cli_design.md): safe `python -m learner_state.selective_prediction_validation` interface design and Step211 implementation status; no Makefile target or release-quality integration is added yet.
...
- `make check-learner-state-selective-prediction`: standalone synthetic selective prediction calibration fixture validator smoke target; not yet connected to release-quality.
- [Selective prediction and calibration release-quality integration design](selective_prediction_calibration_release_quality_integration_design.md): docs-only plan for adding the standalone target after estimator input validation and before config/scoring smoke checks.

...
| Artifact writer CLI actual invocation fixtures | static validator implemented | yes, Step498 fixture root | focused tests | standalone target | no | no | contract docs exist | validator target implemented; actual invocation not implemented |
...
| `tests/fixtures/learner_state_frozen_policy_generation_artifact_writer_cli_actual_invocation/` | artifact writer CLI actual invocation fixture validation | 32 | 6 / 26 | 192 | 6 | Step500 validator module/CLI, focused tests, and Step502 Makefile target | standalone target; not release-quality integrated | metadata-only/body-free sentinel fixtures; no actual invocation correctness claim |

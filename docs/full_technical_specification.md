...
| `check-learner-state-frozen-policy-generation-artifact-writer-cli-actual-invocation-runtime` | runtime CLI v0.2 valid actual invocation metadata-only fixture smoke | Runtime smoke | body-free public-safe v0.2 summary | standalone after Step515; not release-quality integrated yet | `valid/valid_actual_invocation_minimal_metadata_only` | no file writing, no artifact body generation, no manifest writer |

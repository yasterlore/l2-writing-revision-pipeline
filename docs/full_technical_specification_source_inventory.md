...
| Artifact body generation runtime integration plan-only bridge | `frozen_policy_generation_artifact_body_generation_runtime_integration.py` | runtime integration focused tests | existing integration fixture root selected case | Step532-Step537 docs and fixture README | `check-learner-state-frozen-policy-generation-artifact-body-generation-runtime-integration` | none | none | Step535 selected-case public-safe `plan-only-bridge` CLI over `valid/valid_minimal_suppressed_metadata_only_bridge`; Step537 standalone target available; no artifact body runtime invocation, no manifest writer, no file writing, and no wrapper integration yet | yes |

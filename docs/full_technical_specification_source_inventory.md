...
| Artifact body generation runtime invocation fixture root | `frozen_policy_generation_artifact_body_generation_runtime_invocation_fixture_validation.py` | focused validator tests | Step570 planned runtime invocation fixture root | Step569-Step574 docs and fixture README | `check-learner-state-frozen-policy-generation-artifact-body-generation-runtime-invocation-fixtures` | none | none | Step574 adds a standalone Makefile target for validating `tests/fixtures/learner_state_frozen_policy_generation_artifact_body_generation_runtime_invocation/` as 30 metadata-only / body-free cases and 210 JSON files with public-safe CLI output; wrapper integration, runtime implementation, manifest writer integration, and file writing are not implemented | yes |
| Artifact body generation runtime invocation planned-only mode | `frozen_policy_generation_artifact_body_generation_runtime_integration.py` | runtime integration focused tests | Step570 primary valid case | Step575-Step579 docs and fixture README | `check-learner-state-frozen-policy-generation-artifact-body-generation-runtime-invocation` | none | none | Step577 adds selected-case `artifact-body-runtime-invocation` with schema `learner_state_frozen_policy_generation_artifact_body_generation_runtime_integration_v0.3`; Step579 adds a standalone Makefile target for that public-safe metadata-only / body-free smoke; runtime invocation remains planned but not invoked, with no manifest writer invocation and no file writing | yes |

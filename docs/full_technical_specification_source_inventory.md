...
| Artifact writer CLI integration runtime | `frozen_policy_generation_artifact_writer_cli_integration_runtime.py` | runtime focused tests | runtime fixture root | runtime design docs | plan-only target plus Step515 actual invocation metadata-only target | plan-only runtime label only; actual invocation target not wrapper integrated yet | plan-only remote marker only | metadata-only runtime boundary; Step515 standalone v0.2 smoke has no file writing or downstream integration | yes |

...
This component turns generated candidates into simple structural features.

It prepares data for future OT scoring, but it does not score or rank anything yet.
...
Input: one `CandidateSet` per JSONL line.

Output: one `CandidateFeatureSet` per JSONL line.
...
No ranking is used. This component only creates feature records.
...
Feature output excludes candidate descriptions, proposed edit payloads, local context text, and observed edit text.

## 14. Tests added

Tests cover loading CandidateSet JSONL, feature-set construction, forbidden field rejection, post-edit context rejection, leakage flag detection, hold/local/grammar feature classification, text-fragment exclusion, and source scanning for `eval`, `exec`, and `pickle`.

## 15. Known limitations

The features are simple structural descriptors. They are not enough to choose the best candidate.

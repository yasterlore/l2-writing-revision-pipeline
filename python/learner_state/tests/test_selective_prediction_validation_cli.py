...
INVALID_THRESHOLD_CASE = FIXTURE_ROOT / "invalid" / "test_threshold_tuning"


class SelectivePredictionValidationCliTests(unittest.TestCase):
    ...
    def test_invalid_single_case_exits_zero_when_expected_matches(self) -> None:
        completed = run_cli("--fixture-case", str(INVALID_THRESHOLD_CASE))

        self.assertEqual(completed.returncode, 0)
        self.assertIn("validation_status=fail", completed.stdout)
        self.assertIn("reason_codes=test_threshold_tuning", completed.stdout)
        self.assertIn("expected_result_matched=true", completed.stdout)
        assert_safe_cli_output(self, completed)
    ...
    def test_fixture_root_exits_zero_and_reports_all_matches(self) -> None:
        ...
        self.assertIn("total_cases=8", completed.stdout)
        self.assertIn("matched_cases=8", completed.stdout)
    ...
    def test_fixture_root_json_is_parseable_and_safe(self) -> None:
        ...
        self.assertEqual(payload["total_cases"], 8)
        self.assertEqual(payload["matched_cases"], 8)
    ...
    def test_missing_fixture_exits_two(self) -> None:
        completed = run_cli(
            "--fixture-case",
            "tests/fixtures/learner_state_selective_prediction/missing_case",
        )
    ...
    def test_expected_mismatch_exits_three_with_safe_summary(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_case = Path(tmpdir) / "minimal_validation_test_split"
            shutil.copytree(VALID_CASE, tmp_case)
            expected_path = tmp_case / "expected_calibration_validation_result.json"
...
def run_cli(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "learner_state.selective_prediction_validation", *args],
        check=False,
        cwd=Path.cwd(),
        text=True,

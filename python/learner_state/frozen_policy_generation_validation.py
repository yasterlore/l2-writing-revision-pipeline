...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate synthetic frozen policy generation fixtures with safe "
            "reason-code output."
        )
    )
    ...
    mode.add_argument(
        "--fixture-case",
        help="Synthetic frozen policy generation fixture case directory.",
    )
    mode.add_argument(
        "--fixture-root",
        help="Synthetic frozen policy generation fixture root directory.",
    )
    parser.add_argument(
        "--json",
        action="store_true",
...
def _run_cli_fixture_case(case_dir: Path) -> tuple[dict[str, Any], int]:
    ...
    expected_path = case_dir / EXPECTED_GENERATION_RESULT_FILE
...
def _run_cli_fixture_root(root_dir: Path) -> tuple[dict[str, Any], int]:
    ...
    if not root_dir.exists() or not root_dir.is_dir():
        return _input_error_summary(
            "fixture_root",
            FrozenPolicyGenerationValidationFailure(
    ...
    cases = discover_frozen_policy_generation_fixture_cases(root_dir)
    if not cases:
        return _input_error_summary(
            "fixture_root",
            FrozenPolicyGenerationValidationFailure(
                "missing_generation_file",
                "fixture_discovery",
                file_role=EXPECTED_GENERATION_RESULT_FILE,
            ),
        ), 2
...
def _is_usage_like_result(result: FrozenPolicyGenerationValidationResult) -> bool:
    return any(
        reason_code
        in {
            "missing_generation_file",
